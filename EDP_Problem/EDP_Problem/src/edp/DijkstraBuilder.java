...
public class DijkstraBuilder implements Builder {
    ...
    public Solution build(int pos, int numRepm, Solution solution) {
        ...
        for (int j = 0; j < solution.getI().getNodeMatrix().size(); j++) {
            if (!isFirstSolution) {
                int ini = solution.getI().getNodeMatrix().get(j)[0];
                int fin = solution.getI().getNodeMatrix().get(j)[1];
                if (solution.isPairConected(ini, fin)) {
                    continue;
                }
            }
            nodesToDelete = doDijkstra(j, solution);
